...
const Main: React.FC<Props> = ({ product, currencyVal, item }) => {
	...
	const mainDescription =
		product.attributes.paramsWrapper?.mainDescription ||
		product.attributes.paramsWrapper?.aircond?.product?.data.attributes.mainDescription;
	const productBrand =
			...
			<div>
				...
				<div className="mt-5">
						...
						<ul className="mt-4 flex flex-col gap-[10px]">
							{product.attributes.product?.data?.attributes.models.data.map((model, modelIdx) => {
								return (
									...
									>
										<Link
											href={`/catalog/${productTypeSlug}/${model.attributes.slug}`}
					...
					<div className="mt-[30px] flex justify-between items-center">
						<div className="text-[30px] font-semibold md:text-[36px] md:font-medium">
							{(productPrice * currencyVal).toLocaleString()} Сум
						</div>
					...
					<div className="mt-5">
						...
						<a
							...
							className={`mt-[10px] text-center px-2 w-full flex items-center justify-center h-[50px] rounded-[25px] bg-[#6A92FF] text-white text-[16px] duration-300 font-medium hover:bg-[#0f131e]`}
						>
