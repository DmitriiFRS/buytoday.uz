...
export default function TopCard({ product, dollarValue }: { product: AircondProductTypeModel; dollarValue: CurrencyType }) {
   return (
      <Link
         href={`${siteUrl}/catalog/${product.attributes.productType.data.attributes.slug}/${product.attributes.slug}`}
         className={styles.top__col}
         key={product.id}
      >
         <div className={styles.top__img}>
            <Image
               src={
                  strapiUrl + product.attributes.product.data?.attributes.previewImage.data.attributes.url ||
         ...
         <div className={styles.top__colTitle}>
            <div className={styles.top__category}>{product.attributes.productType.data.attributes.title}</div>
         ...
         <div className={styles.top__params}>
            <div className={styles.top__param}>
               Бренд:{" "}
               <span>
                  {product.attributes.product.data?.attributes.brands.data.attributes.title || product.attributes.paramsWrapper.brands.data.attributes.title}
               </span>
            </div>
