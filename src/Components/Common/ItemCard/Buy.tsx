...
import { addItemToCart } from "@/Functions/utilsFunctions";
...
function Buy({ searchTitle, item }: Props) {
   ...
   useEffect(() => {
      ...
      if (items.some((item) => item.model === searchTitle)) {
         items.some((item) => {
            if (item.model === searchTitle) {
               setActiveItem(item);
            }
         });
      } else {
   ...
   return isLoading ? (
      ...
      <button onClick={() => addItemToCart(item, setItem, items)} className={styles.item__buy}>
         Добавить в корзину
      </button>
   );
}
