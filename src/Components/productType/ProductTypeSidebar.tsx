			...
			<div className={`${styles.aircond__aside} ${isMobile ? styles.aircond__aside__mobile : ""}`}>
				{filters[0].attributes.brands.data.length > 0 && (
					<ProductTypeFilterBlock
						title={"Бренды"}
						content={filters[0].attributes.brands.data}
						values={brands}
						setValue={setBrands as () => void}
					/>
				)}
				{filters[0].attributes.btuFilters.data.length > 0 && (
					<ProductTypeFilterBlock
						title={"Мощность"}
						content={filters[0].attributes.btuFilters.data}
						values={btu}
						setValue={setBtu as () => void}
					/>
				)}
				...
				{filters[0].attributes.colors.data.length > 0 && (
					<ProductTypeFilterBlock
						title={"Цвет"}
						content={filters[0].attributes.colors.data}
						values={colors}
						setValue={setColors as () => void}
					/>
				)}
				...
				{filters[0].attributes.dries.data.length > 0 && (
					<ProductTypeFilterBlock
						title={"Сушка"}
						content={filters[0].attributes.dries.data}
						values={dries}
						setValue={setDries as () => void}
					/>
				)}
				...
				{filters[0].attributes.noFrost.data.length > 0 && (
					<ProductTypeFilterBlock
						title={"No Frost"}
						content={filters[0].attributes.noFrost.data}
						values={noFrost}
						setValue={setNoFrost as () => void}
					/>
				)}
			</div>
		)
